...
infix 20 ~_
infix 15 _⊃_ _+_
infix 10 _|≟_ _⊨ᵥ_ _⊭ᵥ_ ⊨ᵥ_ ⊭ᵥ_ _⊨ₘ_ _⊨_ _⊭_ _⊢_ ⊢_ ⊬_
```
...
**〔定义3.5〕** 给定理论 `T` 和公式 `φ`. 对 `T` 的任意模型 `v` 都有 `v ⊨ᵥ φ` 时, 我们就说 `φ` 是 `T` 的逻辑蕴涵 (logical consequence), 也叫语义蕴涵 (semantic consequence), 记作 `T ⊨ φ`. 存在 `T` 的任意模型 `v` 使得 `v ⊭ᵥ φ` 时, 我们就说 `φ` 不是 `T` 的逻辑蕴涵, 记作 `T ⊭ φ`.
...
**〔定义4.5〕** 对任意理论 `T` 和公式 `φ` 都有 `T ⊨ φ` 蕴涵 `T ⊢ φ` 时, 我们就说 $\mathfrak{S}_0$ 具有完全性 (completeness). 对任意理论 `T` 和公式 `φ` 都有 `T ⊢ φ` 蕴涵 `T ⊨ φ` 时, 我们就说 $\mathfrak{S}_0$ 具有可靠性 (soundness). 有些书会把这里说的完全性和可靠性合称为完全性.
