...
强辩护: 对任意理论 `T` 和公式 `φ`, `T ⊨ₜ φ` 当且仅当 `T ⊢ φ`

弱辩护其实是强辩护中 `T ≡ ∅` 的特殊情况. 我们将进行强辩护.

**〔定义4.5〕** 对任意理论 `T` 和公式 `φ` 都有 `T ⊨ₜ φ` 蕴涵 `T ⊢ φ` 时, 我们就说 $\mathfrak{S}_0$ 具有完全性 (completeness). 对任意理论 `T` 和公式 `φ` 都有 `T ⊢ φ` 蕴涵 `T ⊨ₜ φ` 时, 我们就说 $\mathfrak{S}_0$ 具有可靠性 (soundness). 有些书会把这里说的完全性和可靠性合称为完全性.

本篇先证可靠性, 后篇再证完全性.

**[引理4.6]** 如果 `v` 是 `T` 的模型, 那么 `T ⊢ φ` 蕴涵 `v ⊨ φ`.  
证明: (TODO) ∎
